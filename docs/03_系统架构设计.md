...
- **控制器**：使用`ackermann_steering_controller`，自动处理前轮转向角和四轮速度分配
...
|------|----------|------------|------------|------------|
| 侧墙平台 | 4（前2驱动转向 + 后2驱动） | 6（6自由度机械臂） | 10（厚度检测） | IMU, 相机 |
| 底板平台 | 4（前2驱动转向 + 后2驱动） | 6（6自由度机械臂） | 10（厚度检测） | IMU, 相机 |
| 环氧砂浆设备 | 0（无底盘） | 3（简化3自由度） | 12（厚度+压力） | 无IMU/相机 |
...
**作业记录表 (task_record)**：
- id, robot_id, task_type (inspect/grind/spray), start_time, end_time, status (running/completed/failed), progress

**缺陷记录表 (defect_record)**：
- id, task_id, defect_type (crack/bubble/unevenness), position_x/y/z, severity (1-5), image_path, detection_time

**厚度检测表 (thickness_record)**：
- id, task_id, position_x/y, thickness_before, thickness_after, measurement_time

> 详细的表结构SQL、索引设计、数据库迁移策略见 **[05_接口设计文档 - 4.2节](05_接口设计文档.md#42-postgresql表结构)**
...
├── Docker容器 (FAST-LIVO2 - ROS1 Noetic一次性建模)
...
- **数据库接口**：PostgreSQL表结构、InfluxDB时序数据、MinIO对象存储
